...
async function saveAccounts(conn:any,session:any,deviceId:string){let anyBalance=false;
  ...
  for(const a of accounts){
    ...
    let balances:any={};
    try{
      balances=await eb("/accounts/"+encodeURIComponent(uid)+"/balances");
      await audit(deviceId,"bank_balances_ok","account="+uid.slice(0,8)+" count="+(Array.isArray(balances?.balances)?balances.balances.length:0));
    }catch(e){
    ...
    await sql`insert into public.bank_accounts(connection_id,provider_account_id,display_name,iban_last4,currency,owner_name,current_balance,available_balance,last_synced_at)
      ...
      display_name=excluded.display_name,iban_last4=excluded.iban_last4,currency=excluded.currency,owner_name=excluded.owner_name,
      current_balance=coalesce(excluded.current_balance,public.bank_accounts.current_balance),
...
async function fetchTransactions(conn:any,deviceId:string){let anySuccess=false,totalAll=0;
  ...
  for(const a of accounts){
    let key:string|null=null,pages=0,total=0;
    try{
      do{
        const qs=new URLSearchParams({date_from:dateFrom,transaction_status:"BOOK"});
        ...
        for(const t of list){
          ...
          const id=await txKey(a.provider_account_id,t),booked=t?.booking_date?new Date(String(t.booking_date)+"T12:00:00Z"):null;
          await sql`insert into public.bank_transactions(bank_account_id,provider_transaction_id,booked_at,value_date,amount,currency,merchant,description,raw)
            values(${a.id},${id},${booked},${t?.value_date||null},${amount},${String(t?.transaction_amount?.currency||a.currency||"EUR").slice(0,8)},${txMerchant(t,amount)},${txDescription(t)},${sql.json({source:"enablebanking",status:t?.status||"BOOK",merchant_category_code:t?.merchant_category_code||null})})
...
Deno.serve(async (req)=>{
  ...
  try{
    ...
    if(action==="create_link"){
      ...
      const authorizationId=String(authOut.authorization_id||crypto.randomUUID());
      const rows=await sql`insert into public.bank_connections(device_session_id,provider,institution_id,institution_name,requisition_id,status,provider_accounts,consent_expires_at) values(${device.id},'enablebanking',${String(institution.name)},${String(institution.name)},${authorizationId},'PENDING_AUTHORIZATION',${sql.json({state,authorization_id:authorizationId})},${new Date(validUntil)}) returning id`;
      return json({ok:true,connectionId:rows[0].id,link:authOut.url,status:"PENDING_AUTHORIZATION",institution:{name:institution.name}});
    }
    if(action==="authorize"){
      const code=String(body.code||"").trim(),state=String(body.state||"").trim();if(!code||!state)return json({ok:false,error:"missing_callback_parameters"},400);
      const rows=await sql`select * from public.bank_connections where device_session_id=${device.id}::uuid and provider='enablebanking' and revoked_at is null order by created_at desc limit 1`;if(!rows.length)return json({ok:false,error:"no_bank_connection"},404);
      const conn=rows[0],expected=String(conn.provider_accounts?.state||"");if(!expected||state!==expected)return json({ok:false,error:"state_mismatch"},400);
      const session=await eb("/sessions",{method:"POST",body:JSON.stringify({code})}),sessionId=String(session.session_id||"");if(!sessionId)return json({ok:false,error:"missing_session_id"},500);
      const accounts=Array.isArray(session.accounts)?session.accounts:[];
      ...
      const updated={...conn,requisition_id:sessionId,status:"AUTHORIZED",provider_accounts:accounts};
