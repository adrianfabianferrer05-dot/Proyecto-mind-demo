...
function uid(a:any){return typeof a==="string"?a:String(a?.uid||a?.account_uid||a?.id||"")}
function iban(a:any){return typeof a==="string"?"":String(a?.account_id?.iban||a?.iban||"")}
...
async function refreshTarget(targetId:string,sessionId:string,session:any,accounts:any[]){
  ...
  for(const a of accounts){
    ...
    const display=typeof a==="string"?String(match?.display_name||target[0].institution_name||"Cuenta").slice(0,180):String(a?.name||a?.details||a?.product||match?.display_name||target[0].institution_name||"Cuenta").slice(0,180),currency=typeof a==="string"?String(match?.currency||"EUR").slice(0,8):String(a?.currency||match?.currency||"EUR").slice(0,8),owner=typeof a==="string"?(match?.owner_name||null):(a?.name?String(a.name).slice(0,180):(match?.owner_name||null));
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(purpose==="session_refresh"||purpose==="balance_refresh"){
      ...
      const completed={...(conn.provider_accounts||{}),completed_session_id:sessionId,accounts};
      await sql`update public.bank_connections set status='REVOKED',provider_accounts=${sql.json(completed)},revoked_at=now(),updated_at=now() where id=${conn.id}`;
    ...
    for(const a of accounts){const providerId=uid(a);if(!providerId)continue;const accountIban=iban(a),currency=typeof a==="string"?"EUR":String(a?.currency||"EUR").slice(0,8),display=typeof a==="string"?String(conn.institution_name||"Cuenta").slice(0,180):String(a?.name||a?.details||a?.product||conn.institution_name||"Cuenta").slice(0,180);await sql`insert into public.bank_accounts(connection_id,provider_account_id,display_name,iban_last4,currency,owner_name) values(${conn.id},${providerId},${display},${accountIban?accountIban.slice(-4):null},${currency},${typeof a==="string"?null:(a?.name?String(a.name).slice(0,180):null)}) on conflict(connection_id,provider_account_id) do update set display_name=excluded.display_name,iban_last4=excluded.iban_last4,currency=excluded.currency,owner_name=excluded.owner_name,updated_at=now()`;}
